import atexit, os, signal, sys, time, logging
...
class WLANderlustDaemon(object):
  ...
  def __init__(self, foreground):
    ...
    if self.foreground:
      logging.basicConfig(format='%(levelname)-8s %(name)s.%(funcName)s() %(message)s', level=logging.INFO)
    else:
      logging.basicConfig(filename=self.logFile, format='%(asctime)s %(levelname)-8s %(name)s.%(funcName)s() %(message)s', datefmt='%x %X', level=logging.INFO)
